...
public class POIExcelUtil {
	...
	private static String EXCEL_HIDE_SHEET_NAME = "hideSheet"; 
    ...
    public static void creatExcelHidePage(Workbook workbook){   
    	...
    	for(int i=0;i<rootArray.size();i++){
    		...
    		cityList = new String[cityArray.size()];
        ...
        for(int i=0;i<provinceList.length;i++){
        	...
        	for(int j=0;j<cityList.length;j++){
        		Row areaRow = hideInfoSheet.createRow(j+1);
            	creatRow(areaRow, areaDictMap.get(cityList[j]));
        	}
        }
//        //以下行设置区县名称列表
//        for(;count<cityList.length;count++){
//        	Row cityNameRow = hideInfoSheet.createRow(count+1);
//        	creatRow(cityNameRow, areaDictMap.get(cityList[count]));
//        }
//        //名称管理   
//        //第一行设置省份名称列表   
...
//        //以后动态大小设置省份对应的城市列表
//        int nameCount=1;
//        for(;nameCount<provinceList.length;nameCount++){
//        	creatExcelNameList(workbook, provinceList[nameCount], nameCount, areaDictMap.get(provinceList[nameCount]).length, true);
//        } 
//        int m=1;
//        for(int i=0;i<cityList.length;i++,m++){
//        	creatExcelNameList(workbook, cityList[i]+i, m, areaDictMap.get(cityList[i]).length, true);
//        }
        //设置隐藏页标志   
            ...
            for(int i=0;i<sheetIndex;i++){   
                ...
                if(!EXCEL_HIDE_SHEET_NAME.equals(sheet.getSheetName())){
                	...
                	for(int a=2;a<3002;a++){
                        ...
                        //城市选项添加验证数据     
                        data_validation_list = getDataValidationByFormula("INDIRECT(A"+a+")",a,2);   
                        sheet.addValidationData(data_validation_list); 
                        //县城区选项添加验证数据     
...
//		AreaMapper mapper = MapperUtil.getMapper(AreaMapper.class);
//		Area area=new Area();
//		area.setCity("吉安市");
//		List<Area> areaInfo = mapper.getAreaInfo(area);
//		for (Area area2 : areaInfo) {
//			System.out.println(area2.getArea());
//		}
		
		
//		List<String> provinceList = mapper.getProvinceList();
//		for (String string : provinceList) {
//			System.out.println(string);
//		}
		
//		List<String> cityList = mapper.getCityList("江西省",new RowBounds(10,20));
//		for (String string : cityList) {
//			System.out.println(string);
//		}
		
//		List<String> areaList = mapper.getAreaList("吉安市");
